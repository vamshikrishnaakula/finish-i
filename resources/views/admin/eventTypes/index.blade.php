        ...
        <div class="table-wrapper table-responsive">
            <table class="table">
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex">
                            <a class="main-btn primary-btn-outline rounded-md btn-hover me-2" href="{{ route('eventtypes.edit',$type->id) }}"><i class="lni lni-pencil"></i></a>
                            @if($type->status == 'D')
                                <form action="{{ route('eventtypes.activate',$type->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="main-btn danger-btn-outline rounded-md btn-hover" href=""><i class="lni lni-checkmark-circle"></i></button>
                                </form>
                            @else
                                <form action="{{ route('eventtypes.destroy',$type->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
