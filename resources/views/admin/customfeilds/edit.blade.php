    ...
    <script>
        function showfield(name) {
            ...
            if (name == "select") {
                //alert(name)
                document.getElementById("div1").innerHTML =
                    '<br><div class="input-style-1" ><input class="input-style-1" type="text" name="opt[]" required /><br><button type="button" class="btn btn-primary" onclick="adddata()" id="add_text">Add</button></div>';
            } else if (name == "other")
                document.getElementById("div1").innerHTML =
                '<br><div class="input-style-1" ><input class="input-style-1" type="text" name="opt[]" required /><br><button type="button" class="btn btn-primary" onclick="adddata()" id="add_text">Add</button></div>';
        ...
        function adddata() {
            //
            $("#div1").append(
                "<br><div class='input-style-2' ><input type='text' class='form-control' name='opt[]' /><br><button type='button' class='btn btn-danger' onclick='deletedata(event)'id='add_text'>delete</button></div>"
            );
        }
                                ...
                                <div class="select-style-1">
                                    ...
                                    <div class="select-position ">
                                        <select name="Input" id="travel_arriveVia"
                                            ...
                                            <option name="text"{{ $data->InputType == 'text' ? 'selected' : '' }}>text
                                            </option>
                                            <option name="file"{{ $data->InputType == 'file' ? 'selected' : '' }}>file
                                            </option>
                                            <option name="dropdown"{{ $data->InputType == 'dropdown' ? 'selected' : '' }}>
                                                dropdown</option>
                                    ...
                                    @if (
                                        $data->InputType == 'dropdown' or
                                            $data->InputType == 'select' or
                                            $data->InputType == 'radio' or
                                            $data->InputType == 'other')
                                        {{-- <div id="parent_option"> --}}
                                        <div id="div1">
                                            @foreach ($mydata as $options)
                                                <div class='input-style-1'>
                                                   <br><input class='form-control' type='text' name='opt[]'value={{ $options }} /><br><button type='button'class='btn btn-danger' onclick='deletedata(event)'id='add_text'>delete</button>
                                                </div>
                                            @endforeach
                                            <button type='button' class='btn btn-primary' onclick='adddata()'
                                            id='add_text'>Add</button>
                                        </div>
                                    {{-- @else 
                                        <td>hii</td> --}}
